...
class RuntimeDatabase:
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as conn:
                ...
                try:
                    ...
                    await cursor.execute("PRAGMA table_info(runtime_stats)")
                    ...
                    for col_name, col_type in new_columns.items():
                        if col_name not in existing_columns:
                            ...
                            logger.info(
                                f"Added column '{col_name}' to runtime_stats table"
                            )
            ...
            if is_new_database:
                ...
                if not await self._is_migrated():
                    logger.info(
    ...
    async def _is_migrated(self) -> bool:
    ...
    async def _mark_as_migrated(self, imported_count: int):
    ...
    async def _auto_migrate(self):
        ...
        try:
            ...
            if not LOGS_DIR.exists():
                ...
                await self._mark_as_migrated(0)
                return
            ...
            for json_file, mode in json_files:
                ...
                if json_path.exists():
                    try:
                        ...
                        if runtime_data:
                            await self.add_runtime_entry(**runtime_data)
                            imported_count += 1
            ...
            # Mark as migrated
            await self._mark_as_migrated(imported_count)
    ...
    async def get_latest_runtime(self) -> Optional[Dict]:
    ...
    async def get_runtime_history_total_count(self, mode: str = None) -> int:
        """
    ...
    async def get_runtime_stats_summary(self, days: int = 30) -> Dict:
        """
        ...
        try:
            async with aiosqlite.connect(self.db_path) as conn:
                ...
                total_images = (await cursor.fetchone())[0] or 0
                ...
                avg_runtime = (await cursor.fetchone())[0] or 0
                ...
                if latest_row:
                    ...
                    latest_run = {
                        ...
                        "jap_chines_skipped": latest_run_dict.get(
    ...
    async def migrate_runtime_format(self) -> int:
        """
